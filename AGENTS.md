...
- **中文类型**（括号内为等价的英文写法，仅供对照）：
  ...
  | 类型 | 含义 | 对应英文 |
  |------|------|----------|
  | 新增 | **仅用于上架一个全新应用**（该应用第一次进 `apps/`） | feat |
  | 更新 | 已有应用的新功能 / 新内容（新工具、新栏目、新小组件都算） | feat |
  | 修复 | 修 bug | fix |
  ...
  ★ **「新增」不等于「应用里加了东西」**（2026-09-23 用户拍板）：只要这个应用之前已经有提交记录，后续无论加多大的新功能都用「**更新**」，哪怕内容本身是全新的。

  ★★ **写类型前先跑一条命令**（2026-09-23 二次拍板，对所有人和 AI 都强制）：
  `git log --oneline -1 -- apps/<该应用>-*.zip` —— 有输出 = 该应用已有提交史，类型**不允许**「新增」。
  反例（2026-09-23 实际发生）：设置应用加 WebDAV 大功能被写成「新增(设置)」，用户打回——
  原因就是没查这一条、凭「功能是新的」直觉写了「新增」。判据只有一条：**看该应用有没有历史，不看功能新旧**。

- **描述 = 一条更新日志**：写给普通用户看，一句话说清「更新后能感知到什么变化」
  - 不用只有开发者懂的说法（函数名、分片名、`store.json`、TTL、CORS、`pl-14` 之类）；这些细节放正文
  - 末尾带版本号（如 `1.9.0`）；补充说明用 `——` 接在版本号之前
  - 类型已表达动作，描述里再重复一次「新增 / 修复」不算错，但更推荐直接写变化本身（`更新(日历): 「下一个假期」桌面小组件 1.9.0`）
- **正文**（推荐）：用 `- ` 要点写「改了什么 / 为什么这么改 / 怎么验证的（脚本名 + 断言数 + 全绿）」；技术细节、踩坑都放这里
- **scope 仍是应用中文名**（= `store.json` / `builtin-apps.json` 的 `name.zh-CN`）：

  | 类别 | scope |
  |------|-------|
  | 商店应用 | 关注数 · 每日速览 · 书影音游 · 音乐 · 小姐姐视频 · 随机盒子 · 网页屏保 · 朋友圈 · Markdown 笔记 · 番茄钟 · 备忘快帖 · 全网热榜 · 天气 · 日历 · 桌面宠物 · 工具箱 · 小游戏 · 看电视 · 日子 |
